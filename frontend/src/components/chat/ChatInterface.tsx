...
export function ChatInterface({
      ...
      if (error.status === 422) {
        ...
        errorMessage = (
          <>
            ...
            <Link to="/api-keys" className="underline text-blue-600 dark:text-blue-400">API Keys page</Link>.
          </>
        );
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white">
      <ScrollArea className="flex-1 px-6 py-8 bg-gray-100 dark:bg-gray-900">
        <div className="max-w-3xl mx-auto space-y-6">
          {messagesLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-2">
                Type a message below to chat with <span className="text-blue-400 dark:text-blue-600">{selectedModel}</span> via{" "}
                <span className="text-blue-400 dark:text-blue-600">{selectedModel}</span>
              </p>
            </div>
          ) : (
          ...
          {isLoading && !messages?.find((m: Message) => m.role === 'assistant' && m.content === "") && (
            <div className="flex justify-start">
              <div className="bg-gray-200 dark:bg-gray-800 rounded-2xl px-4 py-3 max-w-[80%] shadow-sm">
      ...
      <div className="border-t border-gray-300 dark:border-gray-800 bg-gray-100/95 dark:bg-gray-900/95 backdrop-blur-sm shadow-lg">
        <div className="max-w-3xl mx-auto p-6">
          {apiKeysLoading && (
            <div className="mb-4 p-3 bg-yellow-200/20 dark:bg-yellow-600/20 text-yellow-700 dark:text-yellow-300 rounded-lg">
              Loading API keys...
            </div>
          )}
          {apiKeysError && (
            <div className="mb-4 p-3 bg-red-200/20 dark:bg-red-600/20 text-red-700 dark:text-red-300 rounded-lg">
          ...
          {availableApiKeys.length > 1 && (
            <div className="mb-4">
              <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Select API Key for this chat</label>
              <select
                className="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 focus:ring-2 focus:ring-blue-600 dark:focus:ring-blue-500"
          ...
          {file && (
            <div className="mb-4 p-3 bg-gray-200 dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-700 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <Paperclip className="w-5 h-5 text-gray-600 dark:text-gray-400" />
                <span className="text-sm font-medium text-gray-800 dark:text-gray-200">{file.name}</span>
                <span className="text-xs text-gray-600 dark:text-gray-500">({(file.size / 1024 / 1024).toFixed(1)} MB)</span>
              </div>
              <Button
                variant="ghost"
                size="sm"
                onClick={handleRemoveFile}
                className="h-8 w-8 p-0 hover:bg-gray-300 dark:hover:bg-gray-700 hover:text-red-600 dark:hover:text-red-400 transition-colors duration-200"
              >
                <X className="w-4 h-4" />
              </Button>
            </div>
          ...
          <div className="flex items-end gap-3">
            <Button
              ...
              className="h-12 w-12 p-0 hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors duration-200 rounded-xl"
            >
            ...
            <div className="flex-1 relative">
              <Textarea
                ...
                className="min-h-[48px] max-h-[120px] resize-none bg-gray-200 dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white placeholder-gray-600 dark:placeholder-gray-400 focus:border-blue-600 dark:focus:border-blue-500 focus:ring-blue-600/30 dark:focus:ring-blue-500/30 rounded-xl pr-12 py-3 text-sm font-medium transition-all duration-200"
            ...
            <Button
              ...
              className="flex-shrink-0 h-12 w-12 p-0 bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl transition-colors duration-200"
            >
          ...
          <div className="mt-3 flex items-center justify-center">
            <p className="text-xs text-gray-600 dark:text-gray-400">
              Using <span className="text-blue-400 dark:text-blue-600">{selectedModel}</span> via{" "}
              <span className="text-blue-400 dark:text-blue-600">{selectedProvider}</span>
            </p>
          </div>
        </div>
      </div>
